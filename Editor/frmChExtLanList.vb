...
#Region "画面イベント"
    ...
    ' 機能説明  : 画面表示初期処理を行う
    '--------------------------------------------------------------------
    Private Sub frmChExtLanList_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load

        Try
            'コンボボックス初期設定
            'Ver2.0.2.8 ｸﾘｱのコンボは、iniﾌｧｲﾙではなく自作とする
    ...
    Private Sub cmdMake_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdMake.Click
        ...
        Try
            ...
            ''確認メッセージ
            If MessageBox.Show("Do you make transmission CH data of all ports?", _
                               Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then

                For i As Integer = 0 To gCstCntChSioPort - 1
                    'Ver2.0.0.7 計測点に存在しないﾎﾟｰﾄは消さない版
                    Call gMakeSioTransmissionChData_MakeOnly(i, mudtSetChSioChNew(i))
                Next
    ...
    Private Sub cmdDetails_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDetails.Click
        ...
        Try
            ...
            ''ExtLAN設定詳細画面表示
            If frmChExtLANDetail.gShow(grdSIO.SelectedCells(0).RowIndex, mudtSetChSioNew.shtNum, mudtSetChSioNew.udtVdr, mudtSetChSioChNew, mudtSetChSioExtNew, Me) = 1 Then
...
#Region "内部関数"
    ...
    Private Sub mInitialDataGrid()
        ...
        Try
            ...
            Dim Column3 As New DataGridViewTextBoxColumn : Column3.Name = "txtCommType1" : Column3.ReadOnly = True
            Dim Column4 As New DataGridViewTextBoxColumn : Column4.Name = "txtCommType2" : Column4.ReadOnly = True
            Dim Column5 As New DataGridViewCheckBoxColumn : Column5.Name = "chkJacom55"
            ...
            With grdSIO
                ...
                ''列ヘッダー
                .Columns(0).HeaderText = "LAN Port No."
                .Columns(0).Width = 80
                ...
                .Columns(2).HeaderText = "JACOM-55"
                .Columns(2).Width = 80

                .Columns(3).HeaderText = "COMMType1"
                .Columns(3).Width = 200

                .Columns(4).HeaderText = "COMMType2"
                .Columns(4).Width = 200
                ...
                For i = 0 To .RowCount - 1
                    ...
                    .Rows(i).Cells("txtCommType1").Style.BackColor = gColorGridRowBackReadOnly
                    .Rows(i).Cells("txtCommType2").Style.BackColor = gColorGridRowBackReadOnly
                Next
                ...
                ''ポート2のJACOM-55設定は不可とする
                .Rows(1).Cells(2).ReadOnly = True
                .Rows(1).Cells(2).Style.BackColor = gColorGridRowBackReadOnly
    ...
    Private Sub mSetStructure(ByRef udtSet As gTypSetChSio)
        ...
        Dim iJacomUse As Integer = 0

        Try

            For i As Integer = 0 To grdSIO.Rows.Count - 1

                With udtSet.udtVdr(i + 14) ' 拡張LANは14,15の設定を使用

                    ''Use/Not Useチェックボックスの設定をセット
                    '.shtPort = IIf(grdSIO.Item(1, i).Value, i + 1, 0)
                    .shtPort = IIf(grdSIO.Item(1, i).Value, 1, 0)

                    'ポート14(拡張LANポート1)
                    If i = 0 Then
                        'JACOM55 USEのチェックボックス設定を取得
                        iJacomUse = IIf(grdSIO.Item(2, i).Value, 1, 0)
                        'JACOM55設定をセット
                        mSetJacom55Structure(mudtSetChSioNew, iJacomUse)
                    End If
    ...
    ' 返り値    : なし
    ' 引き数    : ARG1 - ( O) SIO設定構造体
    ' 機能説明  : 構造体にJACOM55設定を格納する
    '--------------------------------------------------------------------
    Private Sub mSetJacom55Structure(ByRef udtSet As gTypSetChSio, iJacomUse As Integer)

        Try

            With udtSet.udtVdr(14)
                If iJacomUse = 1 Then
                    .shtCommType1 = 8
                    .shtCommType2 = 17
                Else
                    'JACOM55を使用しない場合は値を変化させない2019.03.21
                    '.shtCommType1 = 0
                    '.shtCommType2 = 0
                End If
            End With
        ...
        Try
            ...
            For i As Integer = 0 To grdSIO.Rows.Count - 1
                ...
                With udtSet.udtVdr(i + 14) ' 拡張LANは14,15の設定を使用
                    ...
                    ''通信種別１名称
                    strRtn = gGetComboItemName(.shtCommType1, gEnmComboType.ctChSioDetailcmbCommType1, "", False)
                    grdSIO.Item(3, i).Value = IIf(strRtn = "", Hex(.shtCommType1), strRtn)

                    ''通信種別２名称
                    strRtn = gGetComboItemName(.shtCommType2, gEnmComboType.ctChSioDetailcmbCommType2, .shtCommType1, False)
                    grdSIO.Item(4, i).Value = IIf(strRtn = "", Hex(.shtCommType2), strRtn)

                    ''JACOM55チェックボックス
                    If i = 0 Then
                        If .shtCommType1 = 8 And .shtCommType2 = 17 Then
                            grdSIO.Item(2, i).Value = 1
                        Else
                            grdSIO.Item(2, i).Value = 0
                        End If
                    End If
